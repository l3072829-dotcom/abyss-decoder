...
const questions = [
  ...
  { id: 9, story: "你可以消除所有戰爭，代價是全人類喪失『自由意志』，像機器一樣聽命於你。", text: "成為神，還是看著他們互殘？", options: [
    ...
    { label: "做個人。寧可痛苦地自由著。", weights: { S: 10, O: 5, tag: "自由本質" }, whisper: "你允許悲劇發生，只為了守住那虛無縹緲的自由。" }
  ]},
...
export default function App() {
  ...
  const result = useMemo(() => {
    ...
    const confessionTrait = scores.honest === 1 ? "最關鍵的是，你擁有剖析自己偽善的勇氣，這種誠實讓你的人格立體且迷人。" :
  ...
  return (
    <div style={{ backgroundColor: '#000', color: '#fff', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', fontFamily: '-apple-system, sans-serif', padding: '24px', boxSizing: 'border-box', userSelect: 'none' }}>
      {step === 0 && (
        <div style={{ textAlign: 'center', marginTop: '30vh' }}>
          <h1 style={{ fontSize: 'clamp(3rem, 15vw, 5rem)', fontWeight: '900', letterSpacing: '-0.05em', lineHeight: '0.9' }}>ABYSS<br/>DECODER</h1>
        ...
        <div style={{ maxWidth: '500px', width: '100%', marginTop: '8vh' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', opacity: whisper ? 0.3 : 1, pointerEvents: whisper ? 'none' : 'auto' }}>
            {questions[idx].options.map((opt, i) => (
              <button key={i} onClick={() => handleSelect(opt)} style={{ textAlign: 'left', padding: '26px', borderRadius: '20px', backgroundColor: '#0a0a0a', border: '1px solid #1a1a1a', color: '#fff', fontSize: '1.05rem', cursor: 'pointer' }}>{opt.label}</button>
            ))}
          </div>
        </div>
      )}
      {step === 2 && result && (
        <div style={{ textAlign: 'center', maxWidth: '550px', width: '100%', paddingTop: '60px', paddingBottom: '100px' }}>
          ...
          <p style={{ fontSize: '0.9rem', color: '#444', letterSpacing: '2px' }}>{result.logic}</p>
          <button onClick={() => window.location.reload()} style={{ marginTop: '50px', background: 'none', border: 'none', color: '#333', cursor: 'pointer' }}>REBOOT SYSTEM</button>
        </div>
      )}
