...
export default function App() {
  ...
  const finalReport = useMemo(() => {
    ...
    const segments = {
      motive: [
        { cond: P > D && P > S, text: `你基本上是一台會走路的 Excel 表。在你眼裡，甚至連初戀的初吻都能折算成某種社交資本。你對『生存』的嗅覺靈敏得像隻禿鷹。` },
        ...
        { cond: true, text: `你活得像個社會邊緣的幽靈。既不敢徹底學壞，也沒本事變好，只能在中間地帶反覆橫跳。` }
      ],
      mask: [
  ...
  const copyToClipboard = () => {
    const text = `【 Mirror 鏡子：靈魂殘樣診斷 】\n\n${finalReport.p1}\n${finalReport.p2}\n${finalReport.p3}\n\n結論：${finalReport.p4}`;
    navigator.clipboard.writeText(text);
    alert('敢不敢讓朋友們看看真實的你？');
  };

  return (
    <div style={{ backgroundColor: '#1C1C1E', color: '#FFFFFF', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '60px 24px', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif' }}>
      
      {step === 0 && (
        <div style={{ textAlign: 'center', marginTop: '18vh', animation: 'fadeIn 1s' }}>
          <h1 style={{ fontSize: '12px', letterSpacing: '12px', color: '#8E8E93', marginBottom: '24px', fontWeight: '500' }}>MIRROR_SYSTEM_V12.4</h1>
      ...
      {step === 1 && (
        <div style={{ maxWidth: '500px', width: '100%', opacity: isTransitioning ? 0 : 1, transition: '0.4s' }}>
          ...
          <h2 style={{ fontSize: '1.9rem', lineHeight: '1.35', marginBottom: '60px', fontWeight: '700' }}>{questions[idx]?.story}</h2>
      ...
      {step === 2 && finalReport && (
        <div style={{ maxWidth: '640px', width: '100%', animation: 'fadeIn 2s' }}>
          <div style={{ fontSize: '11px', color: '#8E8E93', letterSpacing: '10px', textAlign: 'center', marginBottom: '60px', fontWeight: '500' }}>鏡子：診斷結果</div>
          
          <div style={{ lineHeight: '2.4', fontSize: '1.35rem', textAlign: 'justify', color: '#F2F2F7', background: 'linear-gradient(180deg, rgba(255,255,255,0.06) 0%, rgba(255,255,255,0.01) 100%)', padding: '50px', borderRadius: '32px', border: '1px solid rgba(255,255,255,0.12)', boxShadow: '0 20px 40px rgba(0,0,0,0.3)' }}>
            ...
            <div style={{ color: '#FFFFFF', fontWeight: '900', fontSize: '1.8rem', borderTop: '1px solid #38383A', paddingTop: '36px', marginTop: '36px', lineHeight: '1.35' }}>
              {finalReport.p4}
            </div>
          </div>

          <div style={{ marginTop: '60px', display: 'flex', gap: '20px', justifyContent: 'center' }}>
            <button onClick={copyToClipboard} style={{ padding: '16px 32px', borderRadius: '16px', background: '#3A3A3C', color: '#FFFFFF', border: 'none', fontSize: '15px', fontWeight: '600', cursor: 'pointer' }}>複製診斷</button>
            <button onClick={() => window.print()} style={{ padding: '16px 32px', borderRadius: '16px', background: '#FFFFFF', color: '#000000', border: 'none', fontSize: '15px', fontWeight: '600', cursor: 'pointer' }}>保存截圖</button>
          </div>
      ...
      <style>{`
        ...
        @media print {
          ...
          div { box-shadow: none !important; border: none !important; background: none !important; }
        }
      `}</style>
    </div>
  );
