...
const questions = [
  ...
  { id: 8, story: "整座城市的繁榮，都建立在一個無辜小孩被永世囚禁的痛苦上。只要他離開，所有人的幸福都會崩塌。", text: "你會帶他走嗎？", options: [{ label: "推開地窖門。這幸福令我嘔吐。", weights: { S: 5, O: 4, tag: "理想主義" } }, { label: "關上門離開。犧牲一人是必要的慈悲。", weights: { D: 5, P: 3, tag: "現實維穩" } }] },
  ...
  { id: 12, story: "戰場上，為了救出一師的兄弟，你必須洩漏假情報導致另一個無辜小鎮被轟炸。", text: "聽著無線電的慘叫，你撤退嗎？", options: [{ label: "撤退。軍人的天職是完成任務。", weights: { D: 5, P: 3, tag: "職責枷鎖" } }, { label: "拒援。我不拿無辜者的血換榮譽。", weights: { S: 5, O: 2, tag: "絕對同理" } }] },
  ...
  { id: 20, story: "你發現和平建立在洗腦上。拆穿它，世界將陷入混亂；沈默，則所有人永遠被囚禁在幸福中。", text: "你要真相，還是要安穩？", options: [{ label: "拆穿。虛假的和平只是監獄。", weights: { S: 5, O: 4, tag: "覺醒意志" } }, { label: "沈默。大多數人不需要知道真相。", weights: { D: 5, P: 5, tag: "秩序家" } }] },
  {
    ...
    options: [
      ...
      { label: "「我只是在玩一場遊戲，而我玩得很好。」", weights: { honest: 0.5, tag: "強權者" } }
    ]
  }
];
...
export default function App() {
  ...
  const copyShareLink = (title, logic) => {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(text).then(() => alert("🔗 連結已複製！快分享到 Threads。"));
    } else {
  ...
  const result = useMemo(() => {
    ...
    const confessionTrait = scores.honest === 1 ? "最關鍵的是，你擁有剖析自己偽善的勇氣，這種誠實讓你的人格顯得立體且迷人。" :
    ...
    if (scores.honest === 1) {
      return {
        ...
        content: `你正在經歷一場精心的社會性表演。${coreTrait}你展現的高尚並非源於純粹的利他，而是源於對平庸惡念的極度恐懼。${instinctTrait}${confessionTrait}在那層完美的皮囊下，你比誰都害怕失控，這份「誠實」是你靈魂中唯一的破綻，也是你最美的地方。`
      };
      ...
      return {
        ...
        content: `你渴望超越世俗，拒絕任何既定的標準。${coreTrait}你並不恐懼深淵，你只是恐懼在深淵中發現自己依然平庸。${instinctTrait}${confessionTrait}與其說你在拯救世界，不如說你在挑戰這個世界的極限，這讓你的人性還在拉扯，既痛苦又自由。`
      };
    }
    ...
    <div style={{ backgroundColor: '#000', color: '#fff', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', fontFamily: '-apple-system, system-ui, sans-serif', padding: 'env(safe-area-inset-top) 24px env(safe-area-inset-bottom) 24px', WebkitFontSmoothing: 'antialiased', boxSizing: 'border-box', userSelect: 'none' }}>
      ...
      {step === 2 && result && (
        <div style={{ textAlign: 'center', maxWidth: '550px', width: '100%', paddingTop: '60px', paddingBottom: '100px' }}>
          ...
          <div style={{ backgroundColor: '#080808', border: '1px solid #141414', padding: '45px 30px', borderRadius: '35px', lineHeight: '2.4', fontSize: '1.25rem', fontWeight: '300', textAlign: 'justify', color: '#d1d1d1', marginBottom: '50px' }}>{result.content}</div>
